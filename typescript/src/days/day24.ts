...
export function day24(data : string) {
  const lines = data.split('\n').map(line => {
    const [x0, y0, z0, dx, dy, dz] = line.match(/-?\d+/g).map(Number);
    return { x0, y0, z0, dx, dy, dz }
  })

  const answer1 = 5 // part1(lines);
...
function part2(lines: Line[]) {
  // OK, some equations and we'll start with just X and Y, DX and DY
  ...
  const { x0: x1, y0: y1, dx: dx1, dy: dy1 } = lines[0];
  const { x0: x2, y0: y2, dx: dx2, dy: dy2 } = lines[1];
  const { x0: x3, y0: y3, dx: dx3, dy: dy3 } = lines[2];
  const { x0: x4, y0: y4, dx: dx4, dy: dy4 } = lines[3];
  const { x0: x5, y0: y5, dx: dx5, dy: dy5 } = lines[4];

  const abcde = [dy2 - dy1, dx1 - dx2, y2-y1, x1-x2, x2*dy2 - y2*dx2 - x1*dy1 + y1*dx1];
  const fghij = [dy3 - dy1, dx1 - dx3, y3-y1, x1-x3, x3*dy3 - y3*dx3 - x1*dy1 + y1*dx1];
  const klmno = [dy4 - dy1, dx1 - dx4, y4-y1, x1-x4, x4*dy4 - y4*dx4 - x1*dy1 + y1*dx1];
  const pqrst = [dy5 - dy1, dx1 - dx5, y5-y1, x1-x5, x5*dy5 - y5*dx5 - x1*dy1 + y1*dx1];
...
function solve(abcde: number[], fghij: number[], klmno: number[], pqrst: number[]) {

  console.log(abcde, fghij, klmno, pqrst);
  ...
  console.log([f1, g1, h1, i1, j1], [k1, l1, m1, n1, o1], [p1, q1, r1, s1, t1]);
  ...
  console.log([l2, m2, n2, o2], [q2, r2, s2, t2]);
  ...
  const dy = -t3 / s3;
  const dx = (-s2 * dy - r2) / r2;
  const y0 = (-r1 * dx - s1 * dy - t1) / q1;
  const x0 = (-q * y0 - r * dx - s * dy - t) / p;
