...
function getEnergized(grid: Array2D<string, string>, x0: number, y0: number, dx0: number, dy0: number): number {
    ...
    while (toFollow.length > 0) {
        ...
        while (following) {
            ...
            const prevVisit = visited.get({x, y}) || { up: false, down: false, left: false, right: false };
